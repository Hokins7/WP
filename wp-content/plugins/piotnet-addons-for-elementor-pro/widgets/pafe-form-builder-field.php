...
class PAFE_Form_Builder_Field extends \Elementor\Widget_Base {
	...
	protected function render() {
		...
		$latitude = $settings['google_maps_lat'];
		$longitude = $settings['google_maps_lng'];
		$zoom = $settings['google_maps_zoom']['size'];
					...
					case 'date':
						...
						if ( empty( $item['flatpickr_custom_options_enable'] ) ) {
							...
							if ( ! empty( $item['date_range'] ) ) {
								...
								$this->add_render_attribute( 'input' . $item_index, 'data-pafe-form-builder-date-calculate', '' );
							}
