	...
	function pafeCalculatedFieldsForm() {
        $(document).find('[data-pafe-form-builder-calculated-fields]').each(function(){
            var $fieldWidget = $(this).closest('.elementor-element'),
                ...
                seperatorsSymbol = $fieldCurrent.data('pafe-form-builder-calculated-fields-rounding-decimals-seperators-symbol'),
            ...
            var totalFieldContent = eval(calculation);
        		$fieldWidget.find('.pafe-calculated-fields-form__value').html(round(totalFieldContent, roundingDecimals, decimalsSymbol, seperatorsSymbol, decimalsShow).replace('NaN',0));
				...
				if ( $(this).data('pafe-form-builder-stripe-submit') != undefined ) {
					...
					$fields.each(function(){
						if ( $(this).data('pafe-form-builder-stripe') == undefined && $(this).data('pafe-form-builder-html') == undefined ) {
							...
							if ( !$(this)[0].checkValidity() && $(this).closest('.elementor-widget').css('display') != 'none' && $(this).closest('[data-pafe-form-builder-conditional-logic]').css('display') != 'none' &&  $(this).closest('[data-pafe-signature]').length == 0 ) {
...
jQuery(document).ready(function($) {
		                ...
		                if (type == 'number') {
		                    ...
		                    if(fieldIf != '') {
		                        ...
		                        if($fieldIfSelector.length > 0) {
		                            ...
		                            if(comparison == 'contains') {
	                                	if (fieldIfValue.indexOf('set_value') === -1) {
	                                		error += 1;
	                                    }
	                            	}
		                        }
	...
	function pafeCalculatedFieldsForm(fieldNameElement) {
        ...
        $(document).find(selector).each(function(){
	                ...
	                if ($(this).attr('id') != undefined) {
	                    ...
	                    if($fieldSelector.length > 0) {
	                        ...
	                        if ($repeater_field.length > 0) {
								...
								window['pafe_'+fieldName+'_piotnetpafe'+repeaterIndex+'x'] = parseFloat( fieldValue );
	                        } else {
	                        	window[fieldName] = parseFloat( fieldValue );
	                        }
	                    }
            ...
            if ($repeater.length > 0 && $repeaterAll.length > 0) {
				...
				for (var jx = 0; jx<fieldNameArray.length; jx++) {
					...
					if ($fieldCalc.length > 0) {
						...
						$fieldCalc.each(function(){
							fieldCalcTotal += parseFloat($(this).val());
						});
						var find = fieldName;
        	...
        	if ($(this).attr('data-pafe-form-builder-calculated-fields-coupon-code') != undefined) {
        		...
        		if ($couponCodeFields.length > 0) {
        			...
        			var couponObj = JSON.parse( $couponCodeField.attr('data-pafe-coupon-code-list') );
	...
	$(document).on('keyup change','[data-pafe-form-builder-form-id]', $.debounce( 50, function(){
	...
	function bookingItemLoad() {
		$('[data-pafe-form-booking]').each(function(){
	...
	$(document).on('change paste', '[type="number"][data-pafe-form-builder-form-id]',function(e){
		var val = $(this).val();
...
function clearValidity($this){
    ...
    $firstOption = $parent.find('.elementor-field-option:first-child').eq(0).find('input');
    $firstOption[0].setCustomValidity('');
}