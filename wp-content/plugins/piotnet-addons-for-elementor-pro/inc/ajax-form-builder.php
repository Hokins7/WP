...
', $field['value']) ));
					...
					if (in_array("pdfgenerator", $form['settings']['submit_actions'])) {
						...
						if(!empty($form['settings']['pdfgenerator_title'])){
							...
							$pdf->Cell(219	,5,replace_email($form['settings']['pdfgenerator_title'],$fields,$payment_status, $payment_id),0,1);
							$pdf->Ln(15);
						}
						foreach($pdf_generator_list as $item){
							if($item['custom_font'] == 'yes'){
								$pdf->SetFont('dejaVu','',$item['font_size'] * $pfd_font_ratio);	
							...
							$item_x = ($item['pdfgenerator_set_x']['size'] * 2.11);
							$item_y = ($item['pdfgenerator_set_y']['size'] * 2.97);
							if($item_y < 0){
								$item_y = 0;
							}
							$item_width = $item['pdfgenerator_width']['size'] * 2.19;
							...
							if($type == 'image'){
								...
								$pdf->Image($pdf_image_url, $item_x, $item_y);
							}elseif($type == 'image-upload'){
								$item_width = !empty($item['pdfgenerator_width']) ? $item['pdfgenerator_width']['size'] : '300';
								$item_width = $item_width * 2.1;
								$item_x = $item_x;
								...
								$pdf->Image($pdf_image_url, $item_x, $item_y, $item_width);
							}
							elseif($form['settings']['pdfgenerator_set_custom']=='yes'){
								$pdf->SetXY($item_x, $item_y);
								$pdf->Cell($item_width	,5, replace_email($item['pdfgenerator_field_shortcode'],$fields,$payment_status, $payment_id),0,0);
								$pdf->Cell(100,1,'',0,1);
							}else{
								$pdf->Cell($item_width	,5, replace_email($item['pdfgenerator_field_shortcode'],$fields,$payment_status, $payment_id),0,1);
								$pdf->Cell(100,1,'',0,1);
							}
						}
